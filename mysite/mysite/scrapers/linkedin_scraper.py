...
url = "https://ca.linkedin.com/jobs/search?keywords=software%20developer&location=Nepean%2C%20Ontario%2C%20Canada&geoId=&trk=homepage-jobseeker_jobs-search-bar_search-submit&position=1&pageNum=0"
...
job_links = bsobj.find_all("div", {"class": "base-card relative w-full hover:no-underline focus:no-underline base-card--link base-search-card base-search-card--link job-search-card"})
...
for i in job_links:
	for a in i.find_all('a', href=True):
		...
		if(result.status_code == 200):
			...
			#job title fetch
			job_title = job_bsobj.find("h1", {"class": "top-card-layout__title font-sans text-lg papabear:text-xl font-bold leading-open text-color-text mb-0 topcard__title"}).text
